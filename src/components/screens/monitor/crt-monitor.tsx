...
const OFFICER_ALERT_VIDEOS: Record<string, string> = {
  yuri: "BV1SwGz6tEkR",
  gu:   "BV1SwGz6tEkR",
  lin:  "BV1SwGz6tEkR",
};
...
export function CrtMonitor({
  ...
  const alertVideoUrl = OFFICER_ALERT_VIDEOS[officerId] ?? OFFICER_ALERT_VIDEOS.yuri;
    ...
    try {
      ...
      const detections = await faceapi.detectAllFaces(
        ...
        new faceapi.TinyFaceDetectorOptions({
          inputSize: isMobile ? 160 : 224,  // 手机端更小尺寸，速度快一倍
          scoreThreshold: 0.4,
        })
      );
  ...
  useEffect(() => {
    if (cameraActive && modelLoaded) {
      ...
      if (!cameraActive) { setDetectionStatus("idle"); missingFramesRef.current = 0; alreadyDistractedRef.current = false; }
    }
  ...
  const toggleCamera = async () => {
    if (cameraActive) {
      ...
      setCameraActive(false); setDetectionStatus("idle"); setFaceCount(0);
    } else {
      try {
        // 优先前置摄像头（手机端关键），桌面端也兼容
  ...
  const statusLabel = () => {
    ...
    switch (detectionStatus) {
      case "loading":   return { text: "AI 模型加载中…",               color: "text-yellow-300" };
      case "detecting": return { text: "AI 扫描中…",                   color: "text-yellow-400" };
      case "face-ok":   return { text: `检测到 ${faceCount} 张人脸 ✓`, color: "text-emerald-400" };
      case "no-face":   return { text: "⚠ 未检测到人脸",              color: "text-rose-400" };
      default:          return { text: "初始化中…",                    color: "text-stone-400" };
    }
  };
      ...
      <div className="relative h-64 md:h-80 w-full bg-stone-950">
        <video ref={videoRef}
          className={["absolute inset-0 w-full h-full object-cover",
            cameraActive ? "block" : "hidden"].join(" ")}
          autoPlay playsInline muted
          // iOS WebView 需要这个属性防止强制全屏
        ...
        {!cameraActive && (
          <div className="absolute inset-0 flex flex-col items-center justify-center p-4">
            ...
            <div className="text-emerald-400 flex flex-col items-center gap-2 text-center relative z-10">
              ...
              <p className="text-[11px] px-8 text-stone-500">开启摄像头后，AI 将实时检测人脸，离座或低头时监督官亲自出马</p>
            </div>
          </div>
        )}
        ...
        <div className="absolute inset-0 pointer-events-none"
          style={{ background: "repeating-linear-gradient(0deg,transparent,transparent 2px,rgba(0,0,0,0.1) 2px,rgba(0,0,0,0.1) 4px)", zIndex: 10 }} />

        <div className={["absolute inset-0 pointer-events-none",
          isDistracted ? "shadow-[inset_0_0_50px_rgba(239,68,68,0.45)] bg-red-500/5"
            : "shadow-[inset_0_0_40px_rgba(16,185,129,0.12)]"].join(" ")}
          style={{ zIndex: 11 }} />
        ...
        <div className={["absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28",
          ...
          detectionStatus === "face-ok" ? "border-emerald-400/60"
            : detectionStatus === "no-face" ? "border-rose-500/80" : "border-emerald-500/20"].join(" ")}
          style={{ zIndex: 12 }}>
          <div className={["w-2 h-2 rounded-full transition-colors duration-300",
            detectionStatus === "face-ok" ? "bg-emerald-400"
              : detectionStatus === "no-face" ? "bg-rose-500 animate-ping" : "bg-emerald-400/30"].join(" ")} />
        </div>

        {isDistracted && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="absolute inset-0 flex flex-col" style={{ zIndex: 50 }}>
            <iframe
              key={alertVideoUrl}
            ...
            <div className="bg-black/90 px-3 py-1.5 flex items-center justify-center gap-2 shrink-0">
              <span className="bg-rose-600 text-white font-bangers text-xs px-2 py-0.5 tracking-widest animate-pulse">ZAP! 抓包中！</span>
              <span className="text-rose-400 font-mono text-[10px] truncate max-w-[200px]">{mockEventText}</span>
            </div>
          </motion.div>
        )}

        <div className="absolute top-2 left-2 right-2 flex justify-between items-center text-[10px] font-mono px-2 py-1 bg-black/65 rounded pointer-events-none"
          style={{ zIndex: 30 }}>
          <div className="flex items-center gap-1.5">
            <span className={["w-2 h-2 rounded-full",
              isDistracted ? "bg-rose-500 animate-ping" : "bg-emerald-400 animate-pulse"].join(" ")} />
            <span className="text-emerald-400">AI-TRACKER: LIVE</span>
          </div>
          <span className={`text-[9px] font-bold ${statusColor}`}>{statusText}</span>
          <div className="bg-red-500 text-white font-bold px-1 rounded text-[9px] animate-pulse">REC</div>
        </div>
      </div>
